...
AICR workflow note:
  Run profiling sessions in a b200-devel (or rtx-devel) interactive slot, NOT
  in a b200-batch job.  The batch partitions are shared across six universities.
  Burning an allocation on debugging hurts the whole MGHPCC community.  Profile
  on devel, tune, then submit the polished job to batch.

Usage:
    srun -p b200-devel -N 1 -n 1 -c 4 --mem=16G --gres=gpu:1 --time=01:00:00 --pty bash
...
print(SEP)
print("  PYTORCH PROFILER DEMO — AICR")
print(SEP)
...
print("  (or set up an SSH tunnel from your laptop if on AICR).")
print(SEP)
